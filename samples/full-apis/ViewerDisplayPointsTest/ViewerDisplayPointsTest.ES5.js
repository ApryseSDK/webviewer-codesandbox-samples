      ...
      window.addEventListener('documentLoaded', function() {
        c.initialize().then(function() {
          var t = !1,
            ...
            r = instance.Core.Annotations,
            o = n.getDocument();
          o.getPDFDoc().then(function(s) {
            u && (n.getViewerElement().removeEventListener('mousedown', u), (a = []));
            var l = function(u) {
              if (!t) {
                t = !0;
                var l = instance.Core.documentViewer.getAnnotationManager();
                if (f.length > 0) {
                  for (var h = 0; h < f.length; h++) l.deleteAnnotation(f[h]);
                  f = [];
                }
                console.log('MouseClick X: ' + u.pageX + ', MouseClick Y: ' + u.pageY);
                var p = document.querySelector('.DocumentContainer'),
                  v = p.scrollTop,
                  y = p.scrollLeft,
                  d = { x: u.pageX + y, y: u.pageY + v },
                  g = n
                    .getDisplayModeManager()
                    .getDisplayMode()
                    .getSelectedPages(d, d).first;
                s.requirePage(g)
                  .then(function() {
                    return o.extractPDFNetLayersContext();
                  })
                  .then(function(t) {
            ...
            (u = l), n.getViewerElement().addEventListener('mousedown', l);
          });
        });
      });
  },
