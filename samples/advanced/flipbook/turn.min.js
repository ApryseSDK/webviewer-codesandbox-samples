...
(function($) {
  var pi = Math.PI,
    ...
    turnMethods = {
      init: function(opt) {
        ...
        if (has3d && !isTouch && opt.acceleration) this.transform(translate(0, 0, true));
      ...
      size: function(width, height) {
        if (width && height) {
          ...
          for (p = 1; p <= d.totalPages; p++) {
            ...
            if (d.pages[p]) d.pages[p].css({ width: pageWidth, height: height });
          }
      ...
      view: function(page) {
        var d = this.data(),
          v = turnMethods._visiblePages.apply(this, [page]);
      ...
      page: function(page) {
        ...
        if (pg) {
          if (!d.done || ((!view[0] || page >= view[0]) && (!view[1] || page <= view[1]))) {
            ...
            var o = d.pages[current].data().pageFlip.opt,
              p;
      ...
      resize: function() {
        ...
        for (p = 1; p <= d.totalPages; p++) d.pages[p].flip('resize', true);
      },
      ...
      update: function() {
        ...
        if (d.pageMv.length) {
          ...
          var pos = this.turn('calculateZ', d.pageMv),
            view = this.turn('view', d.tpage),
            apage;
          ...
          for (p = 1; p <= d.totalPages; p++) {
            ...
            if (pos.pageV[p]) d.pages[p].flip('resize');

            if (d.tpage) d.pages[p].flip('disable', p != apage);
          }
        } else {
          // Update static pages
          ...
          for (p = 1; p <= d.totalPages; p++) {
            if ((isFront = p == view[0] || p == view[1])) d.pageWrap[p].css({ 'z-index': d.totalPages, display: '' });
            else if (p == view[0] - 2 || p == view[1] + 2) d.pageWrap[p].css({ 'z-index': d.totalPages - 1, display: '' });
            else d.pageWrap[p].css({ 'z-index': 0, display: 'none' });
      ...
      _pressed: function() {
        ...
        for (p in pages) if (p != page) pages[p].flip('disable', true);
      ...
      disable: function(bool) {
        ...
        for (p = 1; p <= d.totalPages; p++) d.pages[p].flip('disable', bool);
      },
    },
    ...
    flipMethods = {
      ...
      resize: function(full) {
        ...
        if (full) {
          ...
          if (d.opt.frontShadow) d.ashadow.css({ width: height, height: width });

          if (d.opt.backShadow) d.bshadow.css({ width: width, height: height });
        }

        if (d.parent.is(':visible')) {
          d.fwrapper.css({ top: d.parent.offset().top, left: d.parent.offset().left });

          if (d.opt.turn) d.fparent.css({ top: -d.opt.turn.offset().top, left: -d.opt.turn.offset().left });
        }
      ...
      _addPageWrapper: function() {
        ...
        if (!d.wrapper) {
          ...
          d.fpage = $('<div/>', { css: { cursor: 'default' } }).appendTo($('<div/>', divAtt(0, 0, 0, 'visible')).appendTo(d.fwrapper));

          if (d.opt.frontShadow) d.ashadow = $('<div/>', divAtt(0, 0, 1)).appendTo(d.fpage);
            ...
            if (d.opt.backShadow)
              d.bshadow.css({
                'background-image':
                  ...
                  '% ' +
                  gradientEndPointB.y +
                  '%,  color-stop(0.8,rgba(0,0,0,0)), color-stop(1, rgba(0,0,0,' +
                  0.2 * gradientOpacity +
                  ')), to(rgba(0,0,0,0)) )',
              });
          };
      ...
      _moveBackPage: function(bool) {
        ...
        if (d.opt.back)
          if (bool) {
            ...
            if (d.backParent) d.backParent.prepend(d.opt.back);
          }
      },
      ...
      _showThumbIndex: function(c, interpolate) {
        ...
        if (d.opt.back) {
          if (interpolate) {
            var that = this,
              p = d.p || flipMethods._c.call(this, c.corner, 1);
          ...
          if (!d.fwrapper.is(':visible')) {
            ...
            if (d.opt.backShadow) d.bshadow.show();
      ...
      hide: function() {
        ...
        if (--d.fparent.data().flips == 0) d.fparent.hide();
        ...
        if (d.opt.backShadow) d.bshadow.hide();
      ...
      hideThumbIndex: function(interpolate) {
        ...
        if (interpolate) {
          var p2,
            p3,
            p4 = flipMethods._c.call(this, p1.corner),
      ...
      _eventMove: function(e) {
        ...
        if (!d.disabled)
          if (d.cornerActivated) {
            ...
            } else flipMethods.hideThumbIndex.call(this, true);
          }
      },
      ...
      _eventEnd: function() {
        ...
        if (!d.disabled && d.cornerActivated) {
          ...
          if (!event.isPropagationStopped()) flipMethods.hideThumbIndex.call(this, true);
        }
  ...
  $.extend($.fn, {
    ...
    transform: function(t, o) {
      if (o) this.css({ 'transform-origin': o, '-moz-transform-origin': o, '-o-transform-origin': o, '-webkit-transform-origin': o, '-ms-transform-origin': o });
    ...
    animatef: function(p) {
      ...
      if (d.effect) clearInterval(d.effect.handle);

      if (p) {
        if (!p.to.length) p.to = [p.to];
        if (!p.from.length) p.from = [p.from];
        if (!p.easing)
          p.easing = function(x, t, b, c, d) {
        ...
        var j,
          ...
          f = function() {
            ...
            for (j = 0; j < len; j++) v.push(p.easing(1, time, p.from[j], diff[j], p.duration));
        ...
        for (j = 0; j < len; j++) diff.push(p.to[j] - p.from[j]);
